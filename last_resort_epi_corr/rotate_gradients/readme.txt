   ...
   environment variable. 
	...
	A. Locate the name of the environment variable to set, using the 
	 ...
	 NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_Rotate_Gradients_Horiz.sh on UNIX and Mac
...
3. Create packages that contain the following files, based on what
   you are deploying: 
	...
	All Platforms:
		* Rotate_Gradients_Horiz.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
	Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library archive)  
		* Rotate_Gradients_Horiz.exe (application created by MATLAB Compiler)
	UNIX:
		* MCRInstaller.zip (MATLAB Component Runtime library 
	 	  archive - manual unzip required)
		* Rotate_Gradients_Horiz (application created by MATLAB Compiler)
		* run_Rotate_Gradients_Horiz.sh (shell script that can be run 
	...
	All Platforms:            
		* Rotate_Gradients_Horiz.so (shared library, file extension 
		  varies by platform)
		* Rotate_Gradients_Horiz.h (library header file)
		* Rotate_Gradients_Horiz.ctf (Component Technology File archive,
		  platform-dependent on target machine)
	...
	* Rotate_Gradients_Horiz.ctf (Component Technology File archive)	
	* Rotate_Gradients_Horiz.xml (documentation files)
	* Rotate_Gradients_Horiz.pdb (program database file - if 
	  DEBUG option selected)
	* Rotate_Gradients_Horiz.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* Rotate_Gradients_Horiz.ctf (Component Technology File archive)
	* Rotate_Gradients_Horiz_1_0.dll (component containing 
	  compiled M-code)
	...
	* Rotate_Gradients_Horiz.jar (Java archive containing Java 
	  interface to M-code in Rotate_Gradients_Horiz.ctf)
	* Rotate_Gradients_Horiz.ctf (Component Technology File archive,
	  platform-dependent on target machine)
